...
| About (`/about`) | **"2004" founding year horizontal drift + founder portrait + horizontal editorial timeline** — the `2004` watermark drifts left (`xPercent: -28`) via scrub while the founder profile (portrait placeholder, signature, credentials row) reveals on scroll. A full-width pull quote section and a horizontal editorial timeline (5 milestone markers with connecting copper line, same GSAP pin pattern as SouthBay) add cinematic depth unique to the about narrative. Ken Burns breathing zoom (`kenBurns` CSS keyframe, 18s alternate) on the ADU project photo. Sticky credentials panel (CA License + EST. 2004) fades in/out during founder section scroll on desktop. |
