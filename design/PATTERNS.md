...
| Portfolio (`/portfolio`) | **Traveling contact strip** (V4 revamp, 2026-07-13) — each residence case surface carries a decorative row of real working frames that drifts laterally on scrub (xPercent ±7, direction alternating per case), captioned "{residence} / working frames". Content never depends on it (grid/ledger reveals are IO-decisive); it reads as leafing through the photographer's contact sheet. No other page has a lateral-drifting photo strip. |
